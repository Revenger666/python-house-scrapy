...
with open(filename,"r",encoding='utf-8') as f:  # 注意这里一定记得用utf-8打开
    ...
    for i in range(len(pos)):
        if pos[i] == '朝阳':
            ...
            price_sum[0] = price_sum[0] + unit_price[i]
        if pos[i] == '丰台':
            house_num[1] = house_num[1] + 1
            price_sum[1] = price_sum[1] + unit_price[i]
        if pos[i] == '顺义':
            house_num[2] = house_num[2] + 1
            price_sum[2] = price_sum[2] + unit_price[i]
        if pos[i] == '通州':
            house_num[3] = house_num[3] + 1
            price_sum[3] = price_sum[3] + unit_price[i]
        if pos[i] == '大兴':
            house_num[4] = house_num[4] + 1
            price_sum[4] = price_sum[4] + unit_price[i]
        if pos[i] == '昌平':
            house_num[5] = house_num[5] + 1
            price_sum[5] = price_sum[5] + unit_price[i]
        if pos[i] == '门头沟':
            house_num[6] = house_num[6] + 1
            price_sum[6] = price_sum[6] + unit_price[i]
        if pos[i] == '房山':
            house_num[7] = house_num[7] + 1
            price_sum[7] = price_sum[7] + unit_price[i]
        if pos[i] == '密云':
            house_num[8] = house_num[8] + 1
            price_sum[8] = price_sum[8] + unit_price[i]
        if pos[i] == '平谷':
            house_num[9] = house_num[9] + 1
            price_sum[9] = price_sum[9] + unit_price[i]
    print(house_num)
    bins_num = []
    count = 3
    for i in range(0, 11):
    ...
    plt.title('unit_price_show', fontsize=30)
    plt.xlabel('position', fontsize=15)
    plt.ylabel('avg_unit_price/yuan', fontsize=15)
    for i in range(0,10):
